...
func init() {
	android.RegisterModuleType("descendant_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = descendantExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = descendantExpandVariables(ctx, dep_file)
	...
	cmd := descendantExpandVariables(ctx, String(g.properties.Cmd))
